	...
	public function __construct( ) {
		...
		if ( !file_exists( $this->package_dir ) ){ 
			...
			if ( !file_exists( $this->package_dir ) ){ 
				...
				jpcrm_secure_directory_from_external_access( $this->package_dir, true );
			}
	...
	*/
	public function retrieve_and_install_package( $package_key = '', $force_reinstall = false){
		...
		if ( !file_exists( $working_dir ) ){ 
			...
			jpcrm_secure_directory_from_external_access( $working_dir, true );
		}
		$target_dir = $package_info['target_dir'];
		if ( !file_exists( $target_dir ) ) {
			wp_mkdir_p( $target_dir );
			jpcrm_secure_directory_from_external_access( $target_dir, true );
		}
