...
class JPCRM_FeatureSniffer {
  ...
  public function sniff_for_plugin( $args=array(), $is_silent=false ) {
    ...
    if ( !$is_silent && !$is_dismissed && current_user_can( 'activate_plugins' ) ) {
      ...
      $message_template = sprintf( __( 'Your CRM has an optional integration with your <code>%s</code> plugin, but feature is not currently enabled.' ), $plugin_details['Name'] );
      ##WLREMOVE
      $message_template = sprintf( __( 'Jetpack CRM has an optional integration with your <code>%s</code> plugin, but the feature is not currently enabled. Interested? Learn more by going <a href="%s" target="_blank">here</a>.' ), $plugin_details['Name'], $args['more_info_link'] );
      ##/WLREMOVE
      $this->alerts[] = array(
