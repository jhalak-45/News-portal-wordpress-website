						...
						if (!zbscrm_JS_validateEmail(email)) {
							...
							jQuery.ajax({
								...
								success: function(response) {
									...
									window.zbsTestDeliveryMsg = window.zeroBSCRMJS_globViewLang('sendTestSentSuccess') + ' ' + lEmail;
								...
								error: function(response){
									...
									window.zbsTestDeliveryMsg = window.zeroBSCRMJS_globViewLang('sendTestSentFailed');
					...
					success: function(response) {
						...
						swal({
							...
							onClose: function(){
								...
								llIndx = lIndx;
								jQuery('#zbs-mail-delivery-' + llIndx + ' td.zbs-mail-delivery-item-details').prepend('<div class="ui ribbon label zbs-default">' + window.zeroBSCRMJS_globViewLang('defaultText') + '</div>');
		...
		function zeroBSCRMJS_mail_delivery_bindWizard(){
			...
			jQuery('#zbs-mail-delivery-wizard-step-1-submit').off('click').on( 'click', function(){
				...
				if (sendFromName.length > 0){
					...
					// hide any msg
					jQuery('#zbs-mail-delivery-wizard-sendfromname-error').html(window.zeroBSCRMJS_globViewLang('thanks')).addClass('hidden');
					...
					// msg
					jQuery('#zbs-mail-delivery-wizard-sendfromname-error').html(window.zeroBSCRMJS_globViewLang('pleaseEnter')).removeClass('hidden');
				...
				if (sendFromEmail.length > 0 && zbscrm_JS_validateEmail(sendFromEmail)){
					...
					// hide any msg
					jQuery('#zbs-mail-delivery-wizard-sendfromemail-error').html(window.zeroBSCRMJS_globViewLang('thanks')).addClass('hidden');
					...
					// msg
					jQuery('#zbs-mail-delivery-wizard-sendfromemail-error').html(window.zeroBSCRMJS_globViewLang('pleaseEnterEmail')).removeClass('hidden');
			...
			jQuery('#zbs-mail-delivery-wizard-step-2-wrap .ui.radio.checkbox').on( 'click', function(){
				...
				if ( jQuery('#jpcrm-mail-delivery-wizard-step-2-servertype-oauth').checkbox('is checked') ) serverType = 'api';
			...
			jQuery('#zbs-mail-delivery-wizard-step-2-prefill-smtp select').off('change').on( 'change', function(){
				...
				jQuery('#zbs-mail-delivery-wizard-step-2-prefill-smtp select option').each(function(ind,ele){
					...
					if (jQuery(ele).val() == v && window.zbs_root.smtp_providers[v]){
						...
						jQuery('#zbs-mail-delivery-wizard-step-2-smtp-user').attr('placeholder',smtp_settings.example);
						if (!smtp_settings.description) smtp_settings.description = '';
			...
			jQuery('#zbs-mail-delivery-wizard-step-2-submit').off('click').on( 'click', function(){
				...
				if (serverType == "smtp"){
					...
					if (smtpHost.length > 0){
						...
						// hide any msg
						jQuery('#zbs-mail-delivery-wizard-smtphost-error').html(window.zeroBSCRMJS_globViewLang('thanks')).addClass('hidden');
					} else {
						// not okay
						okayToProceed = false;
						// msg
						jQuery('#zbs-mail-delivery-wizard-smtphost-error').html(window.zeroBSCRMJS_globViewLang('pleaseEnter')).removeClass('hidden');
					}
					...
					if (smtpPort.length > 0){
						...
						// hide any msg
						jQuery('#zbs-mail-delivery-wizard-smtpport-error').html(window.zeroBSCRMJS_globViewLang('thanks')).addClass('hidden');
					} else {
						// not okay
						okayToProceed = false;
						// msg
						jQuery('#zbs-mail-delivery-wizard-smtpport-error').html(window.zeroBSCRMJS_globViewLang('pleaseEnter')).removeClass('hidden');
					}
					...
					if (smtpUser.length > 0){
						...
						// hide any msg
						jQuery('#zbs-mail-delivery-wizard-smtpuser-error').html(window.zeroBSCRMJS_globViewLang('thanks')).addClass('hidden');
					} else {
						// not okay
						okayToProceed = false;
						// msg
						jQuery('#zbs-mail-delivery-wizard-smtpuser-error').html(window.zeroBSCRMJS_globViewLang('pleaseEnter')).removeClass('hidden');
					}
					...
					if (smtpPass.length > 0){
						...
						// hide any msg
						jQuery('#zbs-mail-delivery-wizard-smtppass-error').html(window.zeroBSCRMJS_globViewLang('thanks')).addClass('hidden');
					} else {
						// not okay
						okayToProceed = false;
						// msg
						jQuery('#zbs-mail-delivery-wizard-smtppass-error').html(window.zeroBSCRMJS_globViewLang('pleaseEnter')).removeClass('hidden');
					}
				...
				if ( serverType == 'api' ){
					...
					if ( oauth_profile_key ){
						...
						okayToProceed = false;
						jQuery('#zbs-mail-delivery-wizard-oauth-error').html( window.zeroBSCRMJS_globViewLang('oauthConnection') ).removeClass('hidden');
		...
		function zeroBSCRMJS_validateSettings(){
			...
			*/
			...
			switch (serverType){
				...
				jQuery.ajax({
					...
					success: function(response) {
						...
						if (typeof response.success != "undefined"){
							...
							// show result
							var resHTML = window.zeroBSCRMJS_globViewLang('settingsValidatedWPMail') + '<div class="zbs-validated">' + window.zeroBSCRMJS_SMTPWiz.sendFromEmail + '</div>';
				...
				// loading
				jQuery('#zbs-mail-delivery-wizard-validate-console-ico').addClass('loading');
				jQuery('#zbs-mail-delivery-wizard-validate-console').html(window.zeroBSCRMJS_globViewLang('settingsValidateSMTPPortCheck'));
				...
				jQuery.ajax({
					...
					success: function(response) {
						...
						if (typeof response.open != "undefined" && response.open){
							...
							jQuery('#zbs-mail-delivery-wizard-validate-console').html(window.zeroBSCRMJS_globViewLang('settingsValidateSMTPProbing'));
							...
							jQuery.ajax({
								...
								success: function(response) {
									...
									setTimeout(function(){
										...
										jQuery('#zbs-mail-delivery-wizard-validate-console').html(window.zeroBSCRMJS_globViewLang('settingsValidateSMTPAttempt'));
										...
										if (typeof response.success != "undefined" && response.success){
											...
											setTimeout(function(){
												...
												// show result
												var resHTML = window.zeroBSCRMJS_globViewLang('settingsValidatedSMTP') + '<div class="zbs-validated">' + window.zeroBSCRMJS_SMTPWiz.sendFromEmail + '</div>';
												jQuery('#zbs-mail-delivery-wizard-validate-console').html(resHTML);
											...
											// some kind of error, suggest retry
											var resHTML = window.zeroBSCRMJS_globViewLang('settingsValidatedSMTPProbeError');
											jQuery('#zbs-mail-delivery-wizard-validate-console').html(resHTML);
											jQuery('#zbs-mail-delivery-wizard-validate-console-ico').html('<i class="warning sign icon"></i>');
								...
								error: function(response){
									...
									jQuery('#zbs-mail-delivery-wizard-admdebug').html('<strong>Debug Log</strong>:<br />' + window.zeroBSCRMJS_globViewLang('likelytimeout'));
									...
									// some kind of error, suggest retry
									var resHTML = window.zeroBSCRMJS_globViewLang('settingsValidatedSMTPGeneralError');
									jQuery('#zbs-mail-delivery-wizard-validate-console').html(resHTML);
					...
					error: function(response){
						...
						// debug (likely timed out)
						jQuery('#zbs-mail-delivery-wizard-admdebug').html('<strong>Debug Log (Ports Blocked)</strong>:<br />' + window.zeroBSCRMJS_globViewLang('likelytimeout'));
				...
				jQuery.ajax({
					...
					success: function(response) {
						...
						if (typeof response.success != "undefined"){
							...
							// show result
							var resHTML = window.zeroBSCRMJS_globViewLang('settingsValidatedOAuth') + '<div class="zbs-validated">' + window.zeroBSCRMJS_SMTPWiz.sendFromEmail + '</div>';
							jQuery('#zbs-mail-delivery-wizard-validate-console').html(resHTML);
